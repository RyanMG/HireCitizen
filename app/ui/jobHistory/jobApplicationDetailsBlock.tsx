...
export default function JobApplicationDetailsBlock({ application }: { application: JobApplicant }) {
  return (
    <div className="flex flex-row justify-between items-center bg-gray-400 px-4 py-2 my-1 rounded-md">
      ...
      <Link href={application.acceptedStatus === 'PENDING' || application.acceptedStatus === 'REJECTED' ? `/job-list/job/${application.job?.id}` : `/work-history/accepted-jobs/${application.job?.id}`} className="text-sm text-blue">
        View Job
      </Link>
